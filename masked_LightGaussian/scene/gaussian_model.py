...
class GaussianModel:
    ...
    def prune_gaussians(self, percent, import_score: list, mask=None):
        ic(import_score.shape)
        #add
        '''
        if mask != None:
            import_score_nonzero = import_score[import_score.nonzero(as_tuple=True)]
            sorted_tensor, _ = torch.sort(import_score_nonzero, dim=0)
        else:
        '''
        ...
        #add
        if mask != None: 
            prune_mask_nonzero = (mask > 0).squeeze()
            prune_mask = prune_mask_nonzero & prune_mask
        self.prune_points(prune_mask)
