...
def calculate_mask_opacity(opacity_list, mask_gaussian_list):
    opacity_list[(mask_gaussian_list==0).nonzero(as_tuple=True)] = 0
    return opacity_list
...
def prune_list(gaussians, scene, pipe, background):
    ...
    # modify
    gaussian_list, imp_list, mask_gaussian_list, mask_imp_list = None, None, None, None
    viewpoint_cam = viewpoint_stack.pop()
    render_pkg = count_render(viewpoint_cam, gaussians, pipe, background)
    # modify
    gaussian_list, imp_list, mask_gaussian_list, mask_imp_list = (
        render_pkg["gaussians_count"],
        render_pkg["important_score"],
        render_pkg["mask_gaussians_count"], #add
        render_pkg["mask_important_score"], #add    
    )
    ...
    for iteration in range(len(viewpoint_stack)):
        ...
        gaussians_count, important_score, mask_gaussians_count, mask_important_score = (
            render_pkg["gaussians_count"].detach(),
            render_pkg["important_score"].detach(),
            render_pkg["mask_gaussians_count"].detach(), #add
            render_pkg["mask_important_score"].detach(), #add
        )
        gaussian_list += gaussians_count
        imp_list += important_score
        mask_gaussian_list += mask_gaussians_count #add
        mask_imp_list += mask_important_score #add
        gc.collect()
    return gaussian_list, imp_list, mask_gaussian_list, mask_imp_list
