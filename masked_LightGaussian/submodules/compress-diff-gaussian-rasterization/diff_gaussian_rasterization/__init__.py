...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        # add
        gaussians_count, important_score, mask_gaussians_count, mask_important_score, num_rendered, color, radii, geomBuffer, binningBuffer, imgBuffer = None, None, None, None, None, None, None, None, None, None
        # Invoke C++/CUDA rasterizer
        # TODO(Kevin): pass the count in, but the output include a count list 
        if raster_settings.f_count:
            args = args + (raster_settings.f_count,            
                            raster_settings.mask_L, #add
                            raster_settings.mask_R, #add
                            raster_settings.mask_T, #add
                            raster_settings.mask_B, #add   
                            raster_settings.bbd_amo, #add
                            )
            if raster_settings.debug:
                cpu_args = cpu_deep_copy_tuple(args) # Copy them before they can be corrupted
                try:
                    
                    gaussians_count, important_score, mask_gaussians_count, mask_important_score, num_rendered, color, radii, geomBuffer, binningBuffer, imgBuffer = _C.count_gaussians(*args)
        ...
        if raster_settings.f_count: 
            return gaussians_count, important_score, mask_gaussians_count, mask_important_score, color, radii 
        return color, radii
...
class GaussianRasterizationSettings(NamedTuple):
    image_height: int
    image_width: int 
    mask_L: torch.Tensor # add
    mask_R: torch.Tensor # add
    mask_T: torch.Tensor # add
    mask_B: torch.Tensor # add
    bbd_amo: int # add 
    tanfovx : float
    tanfovy : float
    bg : torch.Tensor
