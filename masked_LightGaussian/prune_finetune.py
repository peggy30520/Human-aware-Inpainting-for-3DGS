...
from prune import prune_list, calculate_v_imp_score, calculate_mask_opacity
import torchvision
...
def training(
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if viewpoint_cam.mask is not None: # add
            mask_image = viewpoint_cam.mask.cuda() # add
            if torch.sum(mask_image) != 0:
                mask_image = torch.ones_like(mask_image) - mask_image # add
                gt_image *= mask_image # add
                image *= mask_image # add       
        ...
        with torch.no_grad():
            ...
            if iteration in args.prune_iterations:
                ...
                # modify
                gaussian_list, imp_list, mask_gaussian_list, mask_imp_list = prune_list(gaussians, scene, pipe, background)
                ...
                elif args.prune_type == "mask_v_important_score": # add
                    v_list = calculate_v_imp_score(gaussians, mask_imp_list , args.v_pow)
                    gaussians.prune_gaussians(
                        (args.prune_decay**i) * args.prune_percent, v_list, True
                    )
                elif args.prune_type == "mask_important_score": # add
                    gaussians.prune_gaussians(
                        (args.prune_decay**i) * args.prune_percent, mask_imp_list, True
                    )
                elif args.prune_type == "mask_opacity":
                    opacity_list = calculate_mask_opacity(gaussians.get_opacity.detach(), mask_gaussian_list)
                    gaussians.prune_gaussians(
                        (args.prune_decay**i) * args.prune_percent, gaussians.get_opacity.detach(), mask_gaussian_list
                    )
                elif args.prune_type == "mask_max_v_important_score":
                    v_list = mask_imp_list * torch.max(gaussians.get_scaling, dim=1)[0]
                    gaussians.prune_gaussians(
                        (args.prune_decay**i) * args.prune_percent, v_list , True
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test_iterations", nargs="+", type=int, default=[30_001, 30_002, 40_000, 50_000]
    )
    parser.add_argument(
        "--save_iterations", nargs="+", type=int, default=[40_000, 50_000]
    )
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument(
        "--checkpoint_iterations", nargs="+", type=int, default=[40_000, 50_000]
    )

    parser.add_argument("--prune_iterations", nargs="+", type=int, default=[30_001])
    parser.add_argument("--start_checkpoint", type=str, default="../gaussian-splatting/output/demo/chkpnt30000.pth")
    parser.add_argument("--start_pointcloud", type=str, default=None)
    parser.add_argument("--prune_percent", type=float, default=0.75)
    parser.add_argument("--prune_decay", type=float, default=1)
    parser.add_argument(
        "--prune_type", type=str, default="mask_opacity"
...
'''
python prune_finetune.py -s ../gaussian-splatting/demo -m ../gaussian-splatting/output/demo --iteration 40000 --position_lr_max_steps 40000
'''