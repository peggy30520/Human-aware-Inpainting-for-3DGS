...
def load_image(img_path, mask_path, sigma256=3.0):
    ...
    input_size = min(h, w)
    img = img[:, :, ::-1]
    img = resize(img, input_size, input_size, center_crop=False)
    imgh, imgw = img.shape[0:2]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--img_dir', type=str, help='test image path')
    parser.add_argument('--mask_dir', type=str, help='test mask path')
    ...
    for img, mask in zip(os.listdir(args.img_dir), os.listdir(args.mask_dir)):
        img_path = os.path.join(args.img_dir, img)
        mask_path = os.path.join(args.mask_dir, mask)
        test(model, wf, img_path, mask_path, args.save_path, 0.85, sigma256=3.0)

'''
conda activate train_env
python single_image_test.py --path ckpt/zits_places2_hr --config_file config_list/config_ZITS_HR_places2.yml --GPU_ids '0' --img_dir ../gaussian-splatting/demo/input --mask_dir ../gaussian-splatting/demo/mask --save_path ../gaussian-splatting/demo/inpainted
'''
